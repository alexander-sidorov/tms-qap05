...
def test() -> None:
    ...
    assert (level_04(None)) == {"errors": ["must be date"]}
    assert (level_04(1)) == {"errors": ["must be date"]}
    assert (level_04(1.0)) == {"errors": ["must be date"]}
    assert (level_04(1j)) == {"errors": ["must be date"]}
    assert (level_04(object())) == {"errors": ["must be date"]}
    assert (level_04(object)) == {"errors": ["must be date"]}

    got = level_04(type("_", (), {}))
    assert got == {"errors": ["must be date"]}

    assert (level_04(type)) == {"errors": ["must be date"]}
    assert (level_04(level_04)) == {"errors": ["must be date"]}
    assert (level_04(date(1999, 1, 1))) == {"errors": ["must be date"]}
